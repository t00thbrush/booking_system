...
// ============================
// Teacher Facility Management
// ============================

function assign_facility_to_teacher($teacher_id, $facility_id) {
    global $conn;
    $teacher_id = (int)$teacher_id;
    $facility_id = (int)$facility_id;
    
    $query = "INSERT INTO teacher_facilities (teacher_id, facility_id) VALUES ($teacher_id, $facility_id)
              ON DUPLICATE KEY UPDATE assigned_date = CURRENT_TIMESTAMP";
    return mysqli_query($conn, $query);
}

function revoke_facility_from_teacher($teacher_id, $facility_id) {
    global $conn;
    $teacher_id = (int)$teacher_id;
    $facility_id = (int)$facility_id;
    
    $query = "DELETE FROM teacher_facilities WHERE teacher_id = $teacher_id AND facility_id = $facility_id";
    return mysqli_query($conn, $query);
}

function get_teacher_facilities($teacher_id) {
    global $conn;
    $teacher_id = (int)$teacher_id;
    
    $query = "SELECT f.* FROM facilities f
              JOIN teacher_facilities tf ON f.facility_id = tf.facility_id
...
function get_teacher_facility_ids($teacher_id) {
    global $conn;
    $teacher_id = (int)$teacher_id;
    
    $query = "SELECT facility_id FROM teacher_facilities WHERE teacher_id = $teacher_id";
    $result = mysqli_query($conn, $query);
    $facilities = mysqli_fetch_all($result, MYSQLI_ASSOC);
    return array_map(function($f) { return $f['facility_id']; }, $facilities);
}

function get_all_teachers() {
    global $conn;
    
    $query = "SELECT user_id, name, username, email, is_approved, created_at FROM users 
              WHERE role = 'Teacher' ORDER BY created_at DESC";
    $result = mysqli_query($conn, $query);
    return mysqli_fetch_all($result, MYSQLI_ASSOC);
}

function get_approved_teachers() {
    global $conn;
    
    $query = "SELECT user_id, name, username, email, created_at FROM users 
              WHERE role = 'Teacher' AND is_approved = 1 ORDER BY name ASC";
    $result = mysqli_query($conn, $query);
    return mysqli_fetch_all($result, MYSQLI_ASSOC);
}

function check_teacher_login_status($username) {
    global $conn;
    $username = mysqli_real_escape_string($conn, $username);
